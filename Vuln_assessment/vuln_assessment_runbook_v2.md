...
フェーズ番号は論理グルーピングであり、実施順ではない。原則は「対象を格付け → 各対象で高シグナル検査(P1)を先行 → 精査(P2)→ 網羅・深掘り(P3)→ 補完(P4)」。露出・OSリスク・資産重要度・検査の費用対効果で順序付ける。

### 対象(ホスト)優先順位

Phase 1 の発見後、稼働ホストを以下の順で着手する。

1. 外部到達面(境界・公開サービス)
2. EOL/レガシー OS(Windows 7/2008R2、旧 Solaris、旧 RHEL)
3. 重要資産(DB・ファイルサーバ・バックアップ/管理・インフラ機器)
4. その他の内部ホスト
...
| 優先 | 内容 | 該当フェーズ |
|---|---|---|
| P1 即時・高インパクト | 高シグナル既知脆弱性(MS17-010/BlueKeep/PrintNightmare)、デフォルト/弱資格情報、露出した管理画面・DB・書込可SMB共有、危険レガシーサービス(telnet/rsh/SMBv1) | 2(該当ポート)・3(NSE)・6(露出)・7 |
| P2 高・精査 | credentialed スキャン(OpenVAS)、TLS/SSL 評価、サービス別列挙(SMB/NFS/SNMP/DB)、Web の VCS/設定露出 | 3(GVM)・4・5(露出)・6 |
| P3 中・網羅深掘り | 全TCP/UDP網羅、Web ディレクトリ総当り拡張、ローカル権限昇格列挙、横展開 | 2(全ポート)・5(総当り)・8・9 |
| P4 低・補完 | IPv6/L2 補完発見、低シグナル領域、残差確認 | 1(補完)・随時 |

実施フロー: 対象格付け → 各対象で P1 を先に当て即時リスクを確定 → P2 精査 → P3 網羅・深掘り → P4 補完。検証・トリアージ(Phase 10)と記録(Phase 11)は所見発生の都度、P1 から並行で進める。

> 従来のフェーズ番号順との違いは、弱資格情報(Phase 7)と高シグナル既知脆弱性(Phase 2/3)を全フェーズ完了前に P1 として先行させる点。即時リスクの早期確定と、ノイズの段階的増加(低ノイズ→高ノイズ)を両立する。

---

## 使用ツール優先順位

各能力で「第1選択(高シグナル・低ノイズ・即時)」を基本とし、網羅・credentialed・総当り系は精度が高い分コスト/ノイズも高いため後段に置く。

| 能力 | 第1選択 | 第2・補完 | 選択理由 |
|---|---|---|---|
| ホスト発見 | nmap -sn | masscan(広域)/ arp-scan・netdiscover(L2・ICMP遮断時) | 精度の nmap を基本、広域は masscan、無応答内部は L2 |
| ポート/サービス | nmap -sV -sC | masscan(事前広域スイープ) | バージョン/NSE は nmap、入口の広域だけ masscan |
| 既知脆弱性 | nmap NSE(vuln/vulners) | OpenVAS/GVM(網羅・credentialed) | 軽量NSEで即時(P1)、網羅はGVMで精査(P2) |
| 相関・優先度 | searchsploit + msf check | CISA KEV / EPSS | 実在exploitで裏取り、KEV/EPSSで優先度補正 |
| TLS/SSL | testssl.sh | sslscan / nmap ssl-enum-ciphers | 包括の testssl を基本、軽量確認は sslscan |
| Web 技術特定 | WhatWeb | - | スタック把握の起点 |
| Web ディレクトリ探索 | feroxbuster(再帰) | ffuf(フィルタ柔軟)/ gobuster(代替) | 再帰の ferox を基本、細制御は ffuf |
| Web 脆弱性 | nuclei | nikto(補完)/ wpscan(WordPress時) | テンプレ網羅は nuclei、WPは wpscan |
| SMB 列挙 | NetExec / enum4linux-ng | smbmap -R(共有内容)/ MANSPIDER(機密探索) | 認証・列挙は nxc、内容深掘りは smbmap/manspider |
| SNMP | onesixtyone(community) | snmpwalk(MIB列挙) | community 特定 → 列挙 |
| DB | NetExec(MSSQL)/ ODAT(Oracle) | 各クライアント + nmap NSE | 製品別に第1選択、補完で NSE |
| 弱資格情報 | THC-Hydra | nuclei default-logins(Web既定) | サービス総当りは hydra、Web既定は nuclei |
| ローカル権限昇格 | winPEAS / linPEAS | PowerUp・les(補完)/ WES-NG(欠落パッチ) | 自動列挙が起点。Solaris は unix-privesc-check+手動 |
| 横展開 | NetExec --local-auth | ligolo-ng / chisel(ピボット) | ローカル資格再利用が主、到達はピボット |

原則: 第1選択で低ノイズかつ即時に当たりを取り、網羅・credentialed・総当り系(OpenVAS/全ポート/ディレクトリ総当り/hydra)はレート・タイミング調整のうえ後段で実施する。
...
## Phase 2: ポート/サービス精査【P1 高シグナル / P3 全ポート網羅】
...
## Phase 3: 既知脆弱性 NSE + 統合スキャナ【P1 NSE / P2 GVM】
...
## Phase 4: TLS/SSL 評価(全OS・レガシーで頻出)【P2】
...
## Phase 5: Web アプリ探索【P2 露出 / P3 総当り】
...
## Phase 9: 横展開(AD代替 = ローカルアカウント再利用)【P3】
...
- 整合性修正: 弱資格情報(Phase 7)と高シグナル既知脆弱性(Phase 2/3 の NSE)は本来 P1 だが、フェーズ番号順では中盤以降に位置していた。冒頭の「調査優先順位」で『フェーズ番号=論理グルーピング、実施は対象格付け後に P1 先行』と明示し、即時リスクの早期確定と低ノイズ→高ノイズの段階化を両立させた。
- 段階性: P1(高シグナル・低ノイズ)→ P2(credentialed 精査)→ P3(全ポート網羅・総当り・ローカル深掘り)→ P4(IPv6/L2 補完)の順で、ノイズ増加と検査コストが単調増加するよう配列。OPSEC 方針(広域→該当機→認証後深掘り)と一致。
- ツール整合: 各能力の第1選択はすべて FOSS かつ使用ツール一覧と一致。高コスト系(OpenVAS/全ポート/ディレクトリ総当り/hydra)は P2/P3 に対応づけ済み。
- スコープ整合: AD除外・FOSSのみ・エアギャップ前提と矛盾なし(横展開はローカル資格再利用を第1選択)。
- 残課題: なし。対象格付けは Phase 1 出力(live.txt)に役割・OS・露出の列を付して運用する。
